...
class AdminNotificationService
{
    ...
    {
        $forbidden = ['auth_code', 'auth_code_encrypted', 'password', 'secret', 'api_key', 'token', 'provider_metadata', 'identity_number', 'notes'];
        foreach ($forbidden as $key) {
            unset($payload[$key]);
            unset($payload[$key.'_encrypted']);
        }
        // Deep redact if provider_metadata nested
        if (isset($payload['payload']) && is_array($payload['payload'])) {
            $payload['payload'] = $this->redactPayload($payload['payload']);
        }
        return $payload;
    }
    ...
    public function notifyAdmins(string $type, string $title, string $message, array $payload = []): void
    {
        ...
        $sourceId = $payload['source_id'] ?? $payload['subscription_id'] ?? $payload['invoice_id'] ?? null;
        $dedupeState = $payload['days_left'] ?? $payload['dedupe_state'] ?? null;

        if ($meta) {
            $this->notifyForSource($type, null, array_merge($payload, ['title' => $title, 'message' => $message]), $dedupeState !== null ? (string) $dedupeState : null);
            return;
        }
    ...
    public function notifyRoles(array $roles, string $type, string $title, string $message, array $payload = []): void
    {
        ...
        $dedupeState = $payload['days_left'] ?? null;
        if ($meta) {
            $this->notifyForSource($type, null, array_merge($payload, ['title' => $title, 'message' => $message]), $dedupeState !== null ? (string) $dedupeState : null);
            return;
        }
