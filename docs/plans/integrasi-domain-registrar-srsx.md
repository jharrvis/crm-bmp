...
- **Endpoints SRS-X terkonfirmasi dari KB `kb.srs-x.com`**: `api/domain/updatens` (nameserver comma-separated, min 2), `api/domain/getepp` (`resultData > epp`), `api/domain/setepp` (eppcode 4–16), `api/dns/info` (records `dns0..n` + `domain_ns1..4`), `api/dns/edit` (dnsid, record, type, destination, ttl, priority). Semua POST form-encoded + `api_id` (implementasi placeholder `api_id=1` — validasi UAT). **Tidak ada endpoint WHOIS/contact maupun lock via API** → `manageContacts=false`, lock dilewati.
- **Capabilities**: `updateNameservers=true`, `manageDns=true`, `viewEpp=true`, `manageContacts=false`, `renew=false` (Fase 3), `listDomains=false` (P1 tetap).
...
- **Belum**: `manageContacts` (tidak ada API), lock domain, register/renew (Fase 3), validasi UAT endpoint mutasi produksi (masih gated mode `managed`).
