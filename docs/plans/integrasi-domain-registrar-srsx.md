...
- **Read-only domain detail (2026-08-21)**: `SyncRegistrarDomain` menormalisasi `startdate/enddate` menjadi `registered_at/expires_at`, menyimpan status/ID provider/nameserver, lalu mengambil contact registrant/admin/billing/tech secara read-only. Contact provider disimpan pada `provider_metadata` dan dikecualikan dari Activity Log karena PII. Field secret provider (`authcode`, `auth_code`, `epp`, `eppcode`) dibuang sebelum metadata disimpan; migration `2026_08_21_000010` membersihkan data lama dan `2026_08_21_000011` menyapu key secret dari Activity Log lama. Gagal membaca satu contact tidak menggagalkan sync domain.
