<x-app-layout>
    <div class="space-y-6">
        <div
            ...
            <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
                <div>
                    <h3 class="text-xl font-bold text-slate-800 dark:text-white">IP Transit</h3>
                    <p class="text-slate-500 dark:text-slate-400 text-sm mt-1">Kelola koneksi IP Transit dari vendor.</p>
                </div>
                @can('ip_transits.create')
            ...
            <div class="bg-white dark:bg-slate-800 rounded-3xl shadow-2xl w-full max-w-4xl max-h-[calc(100vh-2rem)] transform scale-95 opacity-0 transition-all duration-300 flex flex-col"
                ...
                <div class="p-6 overflow-y-auto">
                    <form id="dataForm" class="grid grid-cols-1 gap-4 md:grid-cols-2">
                        ...
                        <div>
                            <label class="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">CID (Circuit ID)</label>
                        ...
                        <div>
                            <label class="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">IP Gateway</label>
                            <input type="text" id="ip_gateway" name="ip_gateway"
                                class="w-full rounded-xl border border-slate-200 dark:border-slate-600 px-4 py-2.5 bg-slate-50 dark:bg-slate-700/50 text-slate-800 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all placeholder-slate-400"
                                placeholder="Contoh: 203.0.113.1">
                        </div>

                        <div>
                            <label class="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">AS Number</label>
                            <input type="text" id="as_number" name="as_number"
                                class="w-full rounded-xl border border-slate-200 dark:border-slate-600 px-4 py-2.5 bg-slate-50 dark:bg-slate-700/50 text-slate-800 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all placeholder-slate-400"
                                placeholder="Contoh: AS139967">
                        </div>
    ...
    @push('scripts')
        ...
        <script>
            (function () {
                const baseUrl = '{{ url('/') }}';
                const canUpdate = @json(auth()->user()->can('ip_transits.update'));
                const canDelete = @json(auth()->user()->can('ip_transits.delete'));
                let table;
                ...
                document.getElementById('dataForm').addEventListener('submit', function (e) {
                    ...
                    const isUpdate = !!id;
                    const url = isUpdate ? `${baseUrl}/ip-transits/${id}` : `${baseUrl}/ip-transits`;
                ...
                window.editData = function (id) {
                    fetch(`${baseUrl}/ip-transits/${id}`, {
                        ...
                        .then(item => {
                            ...
                            document.getElementById('as_number').value = item.as_number || '';
                ...
                window.deleteData = function (id) {
                    ...
                    showConfirmModal('Hapus Data?', 'Data tidak dapat dikembalikan.', () => {
                        ...
                        fetch(`${baseUrl}/ip-transits/${deleteId}`, {
                            method: 'POST',
                            headers: {
