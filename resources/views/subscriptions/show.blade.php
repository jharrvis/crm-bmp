                ...
                @if($hostingUsage)
                    <section class="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm dark:border-slate-700 dark:bg-slate-800">
                        <div class="flex flex-col gap-3 border-b border-slate-100 px-6 py-5 sm:flex-row sm:items-center sm:justify-between dark:border-slate-700">
                            ...
                            <a href="{{ route('servers.users.show', ['server' => $subscription->hosting->hostingServer, 'username' => $hostingUsage['user']['username']]) }}" class="inline-flex items-center justify-center gap-2 rounded-lg border border-slate-200 px-3 py-2 text-sm font-semibold text-slate-700 transition hover:bg-slate-50 dark:border-slate-600 dark:text-slate-200 dark:hover:bg-slate-700">
                        ...
                        <div class="grid gap-4 p-6 sm:grid-cols-2 xl:grid-cols-4">
                            <div class="rounded-lg bg-slate-50 p-4 dark:bg-slate-700/40"><p class="text-xs font-bold uppercase tracking-wide text-slate-400">Disk Space</p><p class="mt-2 text-base font-bold text-slate-800 dark:text-white">{{ $formatHostingUsage($hostingUsage['user']['disk_used_mb'], $hostingUsage['user']['disk_quota_mb']) }}</p></div>
                            <div class="rounded-lg bg-slate-50 p-4 dark:bg-slate-700/40"><p class="text-xs font-bold uppercase tracking-wide text-slate-400">Bandwidth</p><p class="mt-2 text-base font-bold text-slate-800 dark:text-white">{{ $formatHostingUsage($hostingUsage['user']['bandwidth_used_mb'], $hostingUsage['user']['bandwidth_quota_mb']) }}</p></div>
                            <div class="rounded-lg bg-slate-50 p-4 dark:bg-slate-700/40"><p class="text-xs font-bold uppercase tracking-wide text-slate-400">Database</p><p class="mt-2 text-base font-bold text-slate-800 dark:text-white">{{ $hostingUsage['user']['databases_count'] ?? 'Tidak tersedia' }}{{ ($hostingUsage['user']['databases_limit'] ?? 0) > 0 ? ' / '.$hostingUsage['user']['databases_limit'] : '' }}</p></div>
                            <div class="rounded-lg bg-slate-50 p-4 dark:bg-slate-700/40"><p class="text-xs font-bold uppercase tracking-wide text-slate-400">Web Domain</p><p class="mt-2 text-base font-bold text-slate-800 dark:text-white">{{ $hostingUsage['user']['web_domains_count'] ?? 'Tidak tersedia' }}{{ ($hostingUsage['user']['web_domains_limit'] ?? 0) > 0 ? ' / '.$hostingUsage['user']['web_domains_limit'] : '' }}</p></div>
                        </div>
                        <dl class="grid gap-x-6 gap-y-4 border-t border-slate-100 px-6 py-5 text-sm sm:grid-cols-2 xl:grid-cols-4 dark:border-slate-700">
                            <div><dt class="text-xs font-bold uppercase tracking-wide text-slate-400">Web</dt><dd class="mt-1 font-semibold text-slate-700 dark:text-slate-200">{{ $formatHostingMb($hostingUsage['user']['disk_web_mb']) }}</dd></div>
                            <div><dt class="text-xs font-bold uppercase tracking-wide text-slate-400">Mail</dt><dd class="mt-1 font-semibold text-slate-700 dark:text-slate-200">{{ $formatHostingMb($hostingUsage['user']['disk_mail_mb']) }}</dd></div>
                            <div><dt class="text-xs font-bold uppercase tracking-wide text-slate-400">Database</dt><dd class="mt-1 font-semibold text-slate-700 dark:text-slate-200">{{ $formatHostingMb($hostingUsage['user']['disk_database_mb']) }}</dd></div>
                            <div><dt class="text-xs font-bold uppercase tracking-wide text-slate-400">Backup / Cron</dt><dd class="mt-1 font-semibold text-slate-700 dark:text-slate-200">{{ $hostingUsage['user']['backups_count'] ?? '-' }} backup · {{ $hostingUsage['user']['cron_jobs_count'] ?? '-' }} cron</dd></div>
                        </dl>
                    </section>
