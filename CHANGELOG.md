...
- **Global Search Quick View — Layanan Pelanggan**: modal quick view hasil pencarian pelanggan kini menampilkan daftar layanan (subscriptions) yang diambil pelanggan. Data: kode langganan, nama paket, nama layanan, nama domain (jika ada), tanggal expired domain (jika ada), status (active/suspended/terminated). Controller `GlobalSearchController` eager load `subscriptions.package.service`. JS render `renderSearchQuickViewSubscriptions()` di `public/assets/js/script.js`.
